# in weighted quick union we'll make sure that always the smaller tree gets
# attached to the larger tree to minimize the average distance
...
class UF_wqusc:
    ...
    def root(self, o):
        ...
        while self.id[o] != o:  # if not different
            # moves 1 extra level for grand parent, simple compress
            self.id[o] = self.id[self.id[o]]
            o = self.id[o]  # move object up 1 level in the tree
        return o  # when matches return


qf = UF_wqusc(10)  # create a qf object with 10 nodes
